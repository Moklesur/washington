...
function washington_breadcrumbs( $args = array() ) {
    ...
    if ( is_singular( 'post' ) ) {
        ...
        $html .= '<span class="item-current item-search"><span class="bread-current bread-search">'.__('Search results for:','washington').' ' . get_search_query() . '</span></span>';
    } elseif ( is_404() ) {
        $html .= '<span>' . __( 'Error 404', 'washington' ) . '</span>';
    } elseif ( is_home() ) {
        $html .= '<span>' . esc_html( get_the_title( get_option( 'page_for_posts' ) ) ) . '</span>';
    }

    $html .= '</div>';
    $html = apply_filters( 'washington_breadcrumbs_filter', $html );
