<div class="camera-slider-widget widget-margin-fix">
	<div class="dhaka-camera-slider">
		<?php foreach( $instance['CameraSlider'] as $i => $CameraSlider ) :
			...
			echo '<div  data-src="'. $image_url .'"><div class="camera-slider-inner '.$CameraSlider['alignment'].'" style="margin:'.$text_position.'"><h1 style="'.$CameraSlider_title_color.'">'.esc_html($CameraSlider['CameraSlider_title']).'</h1><h3 style="'.$CameraSlider_subtitle_color.'">'.esc_html($CameraSlider['CameraSlider_subtitle']).'</h3><div class="slider-details hidden-xs">'.($CameraSlider['CameraSlider_texteditor']).'</div>'.$button.'</div></div>';
